          ...
          <form method="POST" action="{{ url('/password/email') }}" class="needs-validation" novalidate>
            ...
            <div class="content">
              <div class="input-group form-group-no-border input-lg">
                ...
                <input id="email" type="email" class="form-control{{ $errors->has('email') ? ' has-danger' : '' }}" name="email" value="{{ old('email') }}" required autofocus placeholder="{{ trans('validation.attributes.email') }}">
              </div>
              @if ($errors->has('email'))
            ...
            <div class="pull-left">
              <h6>
                <a class="link" href="{{ route('cogroupcms.home') }}">
                  {{ trans('cms.back') }}
                </a>
              </h6>
            </div>
